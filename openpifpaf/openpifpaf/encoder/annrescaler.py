...
class AnnRescalerDet():
    ...
    def detections(self, anns, fpn_interval=None):
        if not fpn_interval is None:
            category_bboxes = []
            for ann in anns:
                if ann['iscrowd'] or not (fpn_interval[0]<=ann['bbox'][2]*ann['bbox'][3]<=fpn_interval[1]):
                    continue
                category_bboxes.append((ann['category_id'], ann['bbox'] / self.stride))
        else:
