...
from openpifpaf.utils import  mask_valid_area, create_sink
...
class CifDet_CN:
    ...
    def __call__(self, image, anns, meta):
        return CifDet_CNGenerator(self)(image, anns, meta)

class CifDet_CNGenerator():
    def __init__(self, config: CifDet_CN):
        self.config = config
    ...
    def init_fields(self, n_fields, bg_mask):
        ...
        # self.fields_reg = np.zeros((n_fields, 2, field_h, field_w), np.nan, dtype=np.float32)
        # self.fields_wh = np.zeros((n_fields, 2, field_h, field_w), np.nan, dtype=np.float32)
        self.fields_reg = np.zeros((n_fields, 2, field_h, field_w), dtype=np.float32)
        self.fields_wh = np.zeros((n_fields, 2, field_h, field_w), dtype=np.float32)
    ...
    def fill_detection(self, f, xy, wh):
        ij = np.round(xy - self.s_offset).astype(int) + self.config.padding
    ...
    def fields(self, valid_area):
        ...
        mask_valid_area(fields_reg[:, 0], valid_area, fill_value=0)
        mask_valid_area(fields_reg[:, 1], valid_area, fill_value=0)
        mask_valid_area(fields_wh[:, 0], valid_area, fill_value=0)
        mask_valid_area(fields_wh[:, 1], valid_area, fill_value=0)
