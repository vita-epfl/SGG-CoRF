...
class VG(Base):
    ...
    def __init__(self, obj_categories, rel_categories, mode, iou_types=['bbox', 'relations'], vg_eval=None):
        ...
        zeroshot_triplet = torch.load("data/visual_genome/zeroshot_triplet.pytorch", map_location=torch.device("cpu")).long().numpy()
    ...
    def _stats_rel(self):
        ...
        result_str += self.evaluator['eval_zeroshot_recall'].generate_print_string(self.mode)
        result_str += self.evaluator['eval_ng_zeroshot_recall'].generate_print_string(self.mode)
