...
from .headmeta import Raf, Raf_dcn
    ...
    def factory(cls, head_metas):
        ...
        return [
            ...
            if (isinstance(meta, headmeta.CifDet)
                and (isinstance(meta_next, Raf) or isinstance(meta_next, Raf_dcn)))
        ]
    @classmethod
    ...
    def __call__(self, fields, initial_annotations=None, meta=None):
        ...
        for raf_v, index_s, x_s, y_s, raf_i, index_o, x_o, y_o in raf_analyzer.triplets[(-raf_analyzer.triplets[:,0]).argsort()]:
            ...
            if (s_idx, o_idx) in dict_rel:
                ...
                cnt = annotations[indx].rel_count[int(raf_i)]
                ...
                if self.max_score:
                    annotations[indx].rel[int(raf_i)] = max(annotations[indx].rel[int(raf_i)], score_rel)
                elif cnt != 0:
                    annotations[indx].rel[int(raf_i)] = (annotations[indx].rel[int(raf_i)]*cnt + score_rel)/(cnt+1)
                else:
                    annotations[indx].rel[int(raf_i)] = score_rel

                if self.add_reverse:
                    indx = dict_rel[(o_idx, s_idx)]-1
                    cnt = annotations[indx].rel_count[int(raf_i)]
                    if not self.pred_dist is None:
                ...
                if s_idx is not None and o_idx is not None:
                    ...
                    ann = AnnotationRaf(self.raf_metas[0].obj_categories,
                                        self.raf_metas[0].rel_categories).set(
                                            ...
                                            category_id_rel=category_id_rel,
                                            score_rel=score_rel, idx_subj=s_idx, idx_obj=o_idx)
                    ...
                    if self.add_reverse:
                        ...
                        ann = AnnotationRaf(self.raf_metas[0].obj_categories,
                                            self.raf_metas[0].rel_categories).set(
                                                obj=annotations_det[s_idx], subj=annotations_det[o_idx],
                                                category_id_rel=category_id_rel,
                                                score_rel=score_rel, idx_subj=o_idx, idx_obj=s_idx)
                        annotations.append(ann)
                        dict_rel[(o_idx, s_idx)] = len(annotations)
