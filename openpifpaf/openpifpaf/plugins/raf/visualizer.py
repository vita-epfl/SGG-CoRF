...
class Raf(Base):
    ...
    def predicted(self, field, ground_truth=None, offset_1=None):
        ...
        if not offset_1 is None:
            self._offsets(offset_1[0, 0:2], offset_1[0, 2:4], annotations=None, confidence_fields=field[:, 0])
    ...
    def _regressions(self, regression_fields1, regression_fields2,
        ...
        for f in f_range:
            ...
            with self.image_canvas(self._processed_image) as ax:
                show.white_screen(ax, alpha=0)
                #ax.text(0, 20, '{}'.format(self.meta.rel_categories[f]), fontsize=14, color='red', bbox={'facecolor': 'black', 'alpha': 0.5, 'linewidth': 0})
                ax.annotate(
                    '{}'.format(self.meta.rel_categories[f]),
                    (10, 20),
                    fontsize=14,
                    xytext=(5.0, 5.0),
                    textcoords='offset points',
                    color='white', bbox={'facecolor': 'black', 'linewidth': 0},
                )
                # if annotations:
                #     self.detection_painter.annotations(ax, annotations, color=('mediumblue', 'blueviolet', 'firebrick'))
                q1 = show.quiver(ax,
                                 ...
                                 xy_scale=self.meta.stride, uv_is_offset=uv_is_offset,
                                 cmap='Reds', clim=(0.1, 0.3), width=0.001, threshold=0.2)
                show.quiver(ax,
                            regression_fields2[f, :2],
                            confidence_field=confidence_fields[f],
                            xy_scale=self.meta.stride, uv_is_offset=uv_is_offset,
                            cmap='Greens', clim=(0.1, 0.3), width=0.001, threshold=0.2)
                if scale_fields1 is not None and scale_fields2 is not None:
                    show.boxes(ax, scale_fields1[f]/2.0,
                               ...
                               xy_scale=self.meta.stride, cmap='Reds', clim=(0.1, 0.3), fill=False,
                    ...
                    show.boxes(ax, scale_fields2[f]/2.0,
                               ...
                               xy_scale=self.meta.stride, cmap='Greens', clim=(0.1, 0.3), fill=False,
                               regression_field_is_offset=uv_is_offset, threshold=0.2)
