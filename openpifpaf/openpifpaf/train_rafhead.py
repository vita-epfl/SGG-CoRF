...
def main():
    ...
    checkpoint = "outputs/hg-210521-041331-365463-vg_cn-rsmooth1.0.pkl.epoch033"
    checkpoint = torch.load(checkpoint)
    net_cpu.base_net = checkpoint['model'].base_net
    net_cpu.head_nets[0] = checkpoint['model'].head_nets[0]
    net_cpu.head_nets[1] = checkpoint['model'].head_nets[1]

    for param in net_cpu.base_net.parameters():
        param.requires_grad = False
    for param in net_cpu.head_nets[0].parameters():
        param.requires_grad = False
    for param in net_cpu.head_nets[1].parameters():
        param.requires_grad = False
    ...
    optimizer = optimize.factory_optimizer(
        args, list(net.module.head_nets[-2].parameters()) + list(net.module.head_nets[-1].parameters()) + list(loss.parameters()))
    start_epoch = 0
